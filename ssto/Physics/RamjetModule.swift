...
//  ssto
//
//  Computes fuel consumption and validates thermal limits for ramjet-powered flight segments
//
//  Operating Limits:
//  - Altitude: 12,500 - 40,000 meters (41,010 - 131,234 feet)
//  - Speed: Mach 2.5 - 6.5
//

import Foundation

/// Module for analyzing ramjet flight segments with thermal, drag, and lift considerations
class RamjetModule {
    ...
    /// Minimum operating altitude for ramjet engines (meters)
    static let minAltitude: Double = 12500.0

    /// Maximum operating altitude for ramjet engines (meters)
    static let maxAltitude: Double = 40001.0
    ...
    /// Check if a ramjet waypoint is thermally safe
    /// - Parameters:
    ...
    static func validateThermalLimits(startWaypoint: Waypoint, endWaypoint: Waypoint,
        ...
        // Check altitude limits for ramjet engines
        ...
        if maxAltitudeReached > maxAltitude {
            let message = String(format: "ALTITUDE LIMIT EXCEEDED!\n\nRamjet engines cannot operate above %.0f meters (%.0f feet).\n\nWaypoint altitude: %.0f meters (%.0f feet)\nExceeded by: %.0f meters\n\nUse scramjet engines at higher altitudes.",
    ...
    /// Simulate a ramjet segment and calculate fuel consumption
    ...
    ///   - planeDesign: Aircraft design parameters
    ///   - propulsion: Ramjet propulsion system
    ...
    static func analyzeSegment(startWaypoint: Waypoint, endWaypoint: Waypoint,
                              ...
                              propulsion: PropulsionSystem) -> RamjetSegmentResult {
        ...
        while iteration < maxIterations {
            ...
            // Clamp altitude to ramjet operating range
            ...
            if mass < PhysicsConstants.dryMass * 0.5 {
                print("WARNING: Excessive fuel consumption in ramjet segment")
                break
            }
        }
    ...
    /// Analyze all ramjet segments in a flight plan
    /// - Parameters:
    ...
    static func analyzeRamjetSegments(waypoints: [Waypoint], currentMass: Double,
                                     planeDesign: PlaneDesign,
                                     propulsion: PropulsionSystem) -> [RamjetSegmentResult] {
        var results: [RamjetSegmentResult] = []
        ...
        for i in 0..<waypoints.count - 1 {
            ...
            // Check if this is a ramjet segment
            if next.engineMode == .ramjet || current.engineMode == .ramjet {
    ...
    /// Print detailed report of ramjet segment analysis
    /// - Parameter results: Array of segment results
    static func printSegmentReport(results: [RamjetSegmentResult]) {
        print("\n========== RAMJET SEGMENT ANALYSIS ==========")

        if results.isEmpty {
            print("No ramjet segments found in flight plan.")
            print("=============================================\n")
            return
        }

        for (index, result) in results.enumerated() {
            print("\nRamjet Segment \(index + 1):")
