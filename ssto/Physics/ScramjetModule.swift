//
//  ScramjetModule.swift
//  ssto
//
//  Computes fuel consumption and validates thermal limits for scramjet-powered flight segments
//
//  Operating Limits:
//  - Altitude: 25,000 - 75,000 meters (82,021 - 246,063 feet)
//  - Speed: Mach 5.0 - 16.0
//

import Foundation

/// Module for analyzing scramjet flight segments with thermal, drag, and lift considerations
class ScramjetModule {
    ...
    /// Minimum operating altitude for scramjet engines (meters)
    static let minAltitude: Double = 25000.0

    /// Maximum operating altitude for scramjet engines (meters)
    static let maxAltitude: Double = 75001.0

    /// Minimum operating speed for scramjet engines (Mach)
    static let minSpeed: Double = 5.0

    /// Maximum operating speed for scramjet engines (Mach)
    ...
    /// Check if a scramjet waypoint is thermally safe
    /// - Parameters:
    ...
    static func validateThermalLimits(startWaypoint: Waypoint, endWaypoint: Waypoint,
        ...
        // Check altitude limits for scramjet engines
        ...
        let minAltitudeReached = min(startAltitudeMeters, endAltitudeMeters)
        let maxAltitudeReached = max(startAltitudeMeters, endAltitudeMeters)

        // Check minimum altitude
        if minAltitudeReached < minAltitude {
            let message = String(format: "ALTITUDE TOO LOW!\n\nScramjet engines cannot operate below %.0f meters (%.0f feet).\n\nWaypoint altitude: %.0f meters (%.0f feet)\nBelow minimum by: %.0f meters\n\nUse ramjet engines at lower altitudes.",
                               minAltitude, minAltitude * PhysicsConstants.metersToFeet,
                               minAltitudeReached, minAltitudeReached * PhysicsConstants.metersToFeet,
                               minAltitude - minAltitudeReached)
            return (false, 0.0, -(minAltitude - minAltitudeReached), message)
        }

        // Check maximum altitude
        if maxAltitudeReached > maxAltitude {
            let message = String(format: "ALTITUDE LIMIT EXCEEDED!\n\nScramjet engines cannot operate above %.0f meters (%.0f feet).\n\nWaypoint altitude: %.0f meters (%.0f feet)\nExceeded by: %.0f meters\n\nUse rocket engines at higher altitudes.",
        ...
        // Check speed limits for scramjet engines
        let minSpeedReached = min(startWaypoint.speed, endWaypoint.speed)
        let maxSpeedReached = max(startWaypoint.speed, endWaypoint.speed)

        // Check minimum speed
        if minSpeedReached < minSpeed {
            let message = String(format: "SPEED TOO LOW!\n\nScramjet engines cannot operate below Mach %.0f.\n\nWaypoint speed: Mach %.1f\nBelow minimum by: Mach %.1f\n\nUse ramjet engines at lower speeds.",
                               minSpeed, minSpeedReached, minSpeed - minSpeedReached)
            return (false, 0.0, -(minSpeed - minSpeedReached), message)
        }

        // Check maximum speed
        if maxSpeedReached > maxSpeed {
            let message = String(format: "SPEED LIMIT EXCEEDED!\n\nScramjet engines cannot operate above Mach %.0f.\n\nWaypoint speed: Mach %.1f\nExceeded by: Mach %.1f\n\nReduce speed to stay within scramjet operating limits.",
                               maxSpeed, maxSpeedReached, maxSpeedReached - maxSpeed)
            return (false, 0.0, -(maxSpeedReached - maxSpeed), message)
        }
    ...
    /// Simulate a scramjet segment and calculate fuel consumption
    ...
    ///   - planeDesign: Aircraft design parameters
    ///   - propulsion: Scramjet propulsion system
    ...
    static func analyzeSegment(startWaypoint: Waypoint, endWaypoint: Waypoint,
                              ...
                              propulsion: PropulsionSystem) -> ScramjetSegmentResult {
        ...
        while iteration < maxIterations {
            ...
            // Clamp altitude to scramjet operating range
            altitude = max(minAltitude, min(altitude, maxAltitude))
            ...
            // Convert to mass (using hydrogen density)
            let fuelMassRate = fuelRate * 0.086 // kg/s (slush hydrogen ~86 kg/m³)
            ...
            if mass < PhysicsConstants.dryMass * 0.5 {
                print("WARNING: Excessive fuel consumption in scramjet segment")
                break
            }
        }
    ...
    /// Analyze all scramjet segments in a flight plan
    /// - Parameters:
    ...
    static func analyzeScramjetSegments(waypoints: [Waypoint], currentMass: Double,
                                       planeDesign: PlaneDesign,
                                       propulsion: PropulsionSystem) -> [ScramjetSegmentResult] {
        var results: [ScramjetSegmentResult] = []
        ...
        for i in 0..<waypoints.count - 1 {
            ...
            // Check if this is a scramjet segment
            if next.engineMode == .scramjet || current.engineMode == .scramjet {
    ...
    /// Print detailed report of scramjet segment analysis
    /// - Parameter results: Array of segment results
    static func printSegmentReport(results: [ScramjetSegmentResult]) {
        print("\n========== SCRAMJET SEGMENT ANALYSIS ==========")

        if results.isEmpty {
            print("No scramjet segments found in flight plan.")
            print("==============================================\n")
            return
        }

        for (index, result) in results.enumerated() {
            print("\nScramjet Segment \(index + 1):")
            ...
            print("  Flight Time: \(String(format: "%.1f", result.timeElapsed)) seconds")
